...
from ocr import reader, read_order_id   # will load EasyOCR ONCE
...
def process_frame(frame: "VideoFrame"):
    ...
    try:
        ...
        try:
            ...
            # if regions() fails, continue (we'll still try OCR)
            pass

        # 3) run OCR to get order id
        # 3) run OCR using external ocr.py
        # global ocr_module
        # if ocr_module is None:
        #     print("[frame_to_minio] Loading external OCR module...")
        #     import ocr                    # <- your working file
        #     ocr_module = ocr
