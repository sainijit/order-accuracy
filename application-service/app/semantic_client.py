...
class SemanticComparisonClient:
    ...
    def semantic_match(
        ...
        logger.info(f"[SEMANTIC-REQUEST-SENT] text1='{text1}', text2='{text2}', context='{context}'")
        ...
        try:
            ...
            semantic_latency = time.time() - semantic_start
            
            if response.status_code == 200:
                result = response.json()
                logger.info(
                    f"[SEMANTIC-RESPONSE-RECEIVED] text1='{text1}', text2='{text2}', "
                    f"match={result.get('match')}, confidence={result.get('confidence', 0.0):.2f}, "
                    f"latency={semantic_latency:.3f}s"
                )
                return result
            else:
                logger.error(
                    f"[SEMANTIC-RESPONSE-ERROR] text1='{text1}', text2='{text2}', "
                    f"status={response.status_code}, latency={semantic_latency:.3f}s"
                )
                return {
                    "match": False,
                    "confidence": 0.0,
            ...
            semantic_latency = time.time() - semantic_start
            logger.error(
                f"[SEMANTIC-RESPONSE-EXCEPTION] text1='{text1}', text2='{text2}', "
                f"error={str(e)}, latency={semantic_latency:.3f}s",
                exc_info=True
            )
            return {
                "match": False,
